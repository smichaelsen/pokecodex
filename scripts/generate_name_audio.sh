...
# Generate German TTS audio for Pokemon names using Piper.
# Uses IDs and name.de/name_tts.de from YAML files and writes to public/audio/de/pokemon/{id}.mp3.
# Existing files are left untouched unless --force is provided.

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
DATA_DIR="$ROOT/data/pokemon"
OUT_DIR="$ROOT/public/audio/de/pokemon"
...
for file in "$DATA_DIR"/*.yml; do
  ...
  name_de="$(get_block_de_value "$file" "name")"
  name_tts="$(get_block_de_value "$file" "name_tts")"
  name_de="$(strip_quotes "${name_de:-}")"
  name_tts="$(strip_quotes "${name_tts:-}")"
  spoken="${name_tts:-$name_de}"

  if [[ -z "${spoken:-}" ]]; then
    echo "Skip $file (no name.de or name_tts.de found)"
    ((failed++)) || true
    continue
  fi
  ...
  tmp_wav="$(mktemp "$CACHE_DIR/tts-${id_line}-XXXX.wav")"
  echo "Generate $dest"
...
echo "Generate name audio done. generated=$downloaded skipped=$skipped failed=$failed"
