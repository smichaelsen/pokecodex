...
# Generate German TTS audio for Pokemon moves using Piper.
# Uses slug + name/tts from data/moves.yml and writes to public/audio/de/moves/{slug}.mp3.
# Existing files are left untouched unless --force is provided.

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
DATA_FILE="$ROOT/data/moves.yml"
OUT_DIR="$ROOT/public/audio/de/moves"
...
flush_move() {
  ...
  local tmp_wav
  tmp_wav="$(mktemp "$CACHE_DIR/tts-move-${slug}-XXXX.wav")"
  echo "Generate $dest"
